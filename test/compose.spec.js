...
describe('testing djantajs packaging compose', () => {
  ...
  before(() => {
    ...
    // flatten the given array map ...
    _.flatten([assembly, nosql]).forEach((p) => {
      pltf.addProject(new Project(manager, p));
    });
  ...
  describe('testing {my-npm-package-name} compose configuration', () => {
    ...
    it('should {my-npm-package-name} project not be local', (done) => {
      ...
      should.exist(project);
      expect(project.isLocal()).to.equal(false);
      assert.equal(project.isLocal(), false,
        'Unexpecting the current project to be a local instance');
    ...
    it('should have {0.2.0} bundle set as default', (done) => {
      ...
      assert.equal(bundle.version, '0.2.0',
        'The version should match with \'0.2.0\'');
    ...
    it('should latest tag bundle', (done) => {
      ...
      assert.equal(bundle.version, '1.0.0',
        'The version should match with \'1.0.0\'');
    ...
    describe('testing bundle {1.0.0} component', () => {
      ...
      it('should bundle {1.0.0} have a valid configuration', (done) => {
        let bundle = pltf.getProject('my-npm-package-name').bundle('1.0.0');
        let json = {
            'my-npm-package-name': {
  ...
  describe('testing the platform project', () => {
    ...
    it('should {my-npm-package-name} project be resolved', (done) => {
      let result = pltf.resolve();
      let finder = name => _.find(result.resolved, bdl => !_.isNil(bdl) &&
        bdl.name === name);
      ...
      should.not.exist(finder('should-be-false@2.0.0'));
      assert.equal(finder('my-npm-package-name@1.0.0').name,
        'my-npm-package-name@1.0.0',
        'The bundle my-npm-package-name@1.0.0 should be results');

      assert.equal(finder('my-testing-nosql@2.0.0').name,
        'my-testing-nosql@2.0.0',
        'The bundle my-npm-package-name@1.0.0 should be results');
