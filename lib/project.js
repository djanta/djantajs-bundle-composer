 ...
 *    constraints: {
 *      os: ['MacOS El-captain', '*'], //Get the name which the current bundle
...
module.exports = class CoreRuntimeProject {
  ...
  constructor (projectManager, definition = {}) {
    ...
    _.each(versions, (value, key) => self.manager
      .register(new Bundle(self, _.defaults({ version: key }, value))));
  }

  /**
   * Sort the provided bundle list within descending order
   * @param {boolean} eligible define the sort eligility
  ...
  get bundles () {
    let self = this;
    return self.manager.bundle(b => !_.isNil(b) &&
      self.name === b.project.name) || [];
  }
  ...
  bundle (filter = void undefined) {
    ...
    let $$0 = _.isFunction(filter) ? filter : _.isNil(filter) ? () => true
